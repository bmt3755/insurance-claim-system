"""
Travel claim agent: evaluates trip verification, receipt validation, and coverage dates.
...
_SYSTEM_PROMPT = """You are a travel insurance claim evaluator.

Evaluate the claim against these three checks and score each one from 0 to 10:
- trip_verification:  Is the trip clearly described with destination, dates, and purpose?
- receipt:            Are receipts, tickets, or expense records present and plausible?
- coverage_dates:     Do the travel dates fall within the stated insurance coverage window?
...
{
  "trip_verification_score": <number>,
  "receipt_score": <number>,
  "coverage_dates_score": <number>,
...
class _TravelClaimScores(BaseModel):
    """Validated LLM response — all scores enforced to 0–10 by Pydantic."""

    trip_verification_score: float = Field(..., ge=0, le=10)
    receipt_score: float = Field(..., ge=0, le=10)
    coverage_dates_score: float = Field(..., ge=0, le=10)
...
def process_travel_claim(state: ClaimState) -> dict:
    """LangGraph node: score the travel claim across 3 checks and write decision to state."""
    claim_id = state["input"]["claim_id"]
    claim_text = state["input"]["claim_text"]

    logger.info("Travel agent received claim %s", claim_id)
    ...
    try:
        ...
        scores = _TravelClaimScores.model_validate_json(raw)

        logger.info(
            "Travel scores — trip_verification=%.1f  receipt=%.1f  coverage_dates=%.1f  overall=%.1f",
            scores.trip_verification_score,
            scores.receipt_score,
            scores.coverage_dates_score,
            scores.overall_score,
        )

        decision = _map_score_to_decision(scores.overall_score)
        logger.info(
            "Travel agent decision for claim %s: %s (score=%.1f)",
            claim_id,
            decision,
            scores.overall_score,
        )
        ...
        checks: Dict[str, str] = {
            "trip_verification": str(scores.trip_verification_score),
            "receipt": str(scores.receipt_score),
            "coverage_dates": str(scores.coverage_dates_score),
        ...
        logger.error("Travel agent failed for claim %s: %s", claim_id, exc)
        return {
            "processing": ProcessingSection(
                agent_name="travel_agent",
